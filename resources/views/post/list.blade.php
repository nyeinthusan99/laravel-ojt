...
@section('content')
    <div class="container mt-5">
        <div class="row d-flex">
            ...
            <form action="{{ route('postlist') }}" method="GET" class="col-md-6 d-flex">
                ...
                <div class="col-md-8 me-2">
                    <input type="text" value="{{ request('searchItem') }}"
                        placeholder="Search title, description or created user" class="form-control" name="searchItem"
                        id="searchitem">
                </div>
                <div class="col-md-4">
            ...
            {{-- create button --}}
            <div class="col-md-2">
                <a href="{{ route('post.create') }}" class="btn btn-primary form-control"><i
                        class="fa-solid fa-circle-plus"></i> Create Post</a>
            </div>
            {{-- upload & download --}}
            <div class="col-md-2">
                <a href="{{ route('post.upload') }}" class="btn btn-primary form-control"><i
                        class="fa-sharp fa-solid fa-upload"></i> Upload</a>
            </div>
            <form action="{{ route('post.export') }}" method="post" class="col-2">
                @csrf
                <button class="btn btn-primary form-control" {{ $posts->isEmpty() ? 'disabled' : '' }}><i
        ...
        <table class="table table-bordered m-auto w-75 mt-5" id="mytable">
            ...
            <tbody>
                ...
                @foreach ($posts as $post)
                    <tr>
                        <td scope="row">
                            <a href="" class="post-ttl" data-bs-toggle="modal"
                                data-bs-target="#detail{{ $post->id }}">
                                {{ $post->title }}
                            </a>
                        </td>
                        ...
                        <td>
                            <a href="/post/update/{{ $post->id }}" class="btn btn-warning btn-sm form-control"><i
                        ...
                        <td>
                            <button type="button" class="btn btn-danger btn-sm form-control" data-bs-toggle="modal"
                                data-bs-target="#Delete{{ $post->id }}"><i class="fa-solid fa-trash"></i>
                                Delete</button>
                        </td>
                    </tr>
                @endforeach
        ...
        <!-- Detail Modal -->
        @foreach ($posts as $post)
            <div class="modal fade" id="detail{{ $post->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                        ...
                        <div class="modal-body">
                            <div class="col-md-12">
                                <div class="row mb-2">
                                    ...
                                    <label class="col-md-8 post-ttl">{{ $post->title }}</label>
                                </div>
                                <div class="row mb-2">
        ...
        <!-- Delete Modal -->
        @foreach ($posts as $post)
            <div class="modal fade" id="Delete{{ $post->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
